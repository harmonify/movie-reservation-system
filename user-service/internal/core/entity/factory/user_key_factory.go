package entityfactory

import "github.com/harmonify/movie-reservation-system/user-service/internal/core/entity"

type UserKeyFactory interface {
	GenerateUserKey(*entity.User) *entity.UserKey
}

type userKeyFactoryImpl struct{}

func NewUserKeyFactory() UserKeyFactory {
	return &userKeyFactoryImpl{}
}

func (f *userKeyFactoryImpl) GenerateUserKey(user *entity.User) *entity.UserKey {
	return &entity.UserKey{
		UserUUID:   user.UUID,
		PublicKey:  PUBLIC_KEY,
		PrivateKey: PRIVATE_KEY,
		CreatedAt:  user.CreatedAt,
		UpdatedAt:  user.UpdatedAt,
		DeletedAt:  user.DeletedAt,
	}
}

const (
	PUBLIC_KEY = ""

	PRIVATE_KEY = ".Iy1+Zux/wvz//kT2xW4fjbkawdwt9X4YJlpjpkCZ7oo=.a1VorD0gFYz9YL3D.15000"
)
